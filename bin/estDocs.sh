...
now=`date`
echo $now >> $logFile
echo "hadoop jar $JAR $MAIN  --input $inputDocs -k 5 --output ${rootPath}/models --maxIter 40 -mipd 1 --dictionary ${rootPath}/dictionary/dict \
...
hadoop jar $JAR $MAIN  --input $inputDocs -k 5 --output ${rootPath}/models --maxIter 40 -mipd 1 --dictionary ${rootPath}/dictionary/dict \
-dt ${rootPath}/docTopics/est -mt ${rootPath}/tmpModels --num_reduce_tasks 1 --num_train_threads 8 --num_update_threads 4 --test_set_fraction 0.1 --iteration_block_size 5 >> $logFile 2>&1
